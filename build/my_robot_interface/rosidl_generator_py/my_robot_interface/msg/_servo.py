...
class Servo(metaclass=Metaclass_Servo):
    ...
    _fields_and_field_types = {
        'servo_states': 'int64[2]',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('int64'), 2),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        if 'servo_states' not in kwargs:
            self.servo_states = numpy.zeros(2, dtype=numpy.int64)
        else:
            self.servo_states = numpy.array(kwargs.get('servo_states'), dtype=numpy.int64)
    ...
    def servo_states(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.int64, \
                "The 'servo_states' numpy.ndarray() must have the dtype of 'numpy.int64'"
            assert value.size == 2, \
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'servo_states' field must be a set or sequence with length 2 and each value of type 'int' and each integer in [-9223372036854775808, 9223372036854775807]"
        self._servo_states = numpy.array(value, dtype=numpy.int64)
